...
import jobManagerContractArtifact from "../contracts/JobManager.json";
import AIModelOwnershipContractArtifact from "../contracts/AIModelOwnership.json";
...
const NETWORK_ID = "10200";

// This is an error code that indicates that the user canceled a transaction
const ERROR_CODE_TX_REJECTED_BY_USER = 4001;
const IPFS_URL = "https://ipfs.chainsafe.io/ipfs/";
...
export class Dapp extends React.Component {
  constructor(props) {
    ...
    // staking value in eth
    this.stakeValue = "0.1";
    // the number of eth customer needs to pay for each call
    this.payValue = "0.01";
  }
  ...
  async _initializeEthers() {
    ...
    this._contract = new ethers.Contract(
      contractAddress.JobManager,
      jobManagerContractArtifact.abi,
      this._provider.getSigner(0)
    );

    this._modelOwnershipContract = new ethers.Contract(
  ...
  async _unstake() {
    try {
      console.log("entering unstake function");
      this._dismissTransactionError();
      const tx = await this._contract.unstake();
  ...
  async _stake() {
    try {
      ...
      // clear it.
      console.log("entering stake function");
      this._dismissTransactionError();
      const options = { value: ethers.utils.parseEther(this.stakeValue) };
      const tx = await this._contract.stake(options);
      this.setState({ txBeingSent: tx.hash });
